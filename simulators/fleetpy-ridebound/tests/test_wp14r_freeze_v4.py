...
_FREEZE_V4 = _ADAPTER / "wp14r_freeze_v4.py"
_RECEIPT = (
    _REPOSITORY
    / "benchmarks/scenarios/wp14r-development/freeze-v4-authorization.json"
)

_spec = importlib.util.spec_from_file_location("wp14r_freeze_v4_under_test", _FREEZE_V4)
...
class FreezeV4Tests(unittest.TestCase):
    def setUp(self):
        self.assertTrue(_RECEIPT.is_file(), "freeze v4 receipt must be built")
        self.receipt = json.loads(_RECEIPT.read_text(encoding="utf-8"))

    def test_receipt_is_superseded_and_no_longer_rebuildable(self):
        """Freeze v4 is a retained record: it reached the gate but could not sign it.

        Under v4 both paired jobs succeeded and both verified independently — the
        first time that gate was ever reached. Writing the gate receipt then
        failed because the paired-gate schema pinned `freezeId` to the literal v2
        identifier. Freeing that field changed a file the v4 receipt binds, so v4
        can no longer be rebuilt. The live regression lives in
        `test_wp14r_freeze_v5.py`.
        """
        completed = subprocess.run(
            [
                sys.executable, "-B", str(_FREEZE_V4),
    ...
    def test_the_retained_receipt_bytes_are_untouched(self):
        ...
        self.assertEqual(
            ...
            "f7acbe2d3d021404c67dcadea5d524a75c7e367e21a5da5da218731862cc142b",
        )
    ...
    def test_live_roots_are_v4_literal_paths(self):
    ...
    def test_identity_is_v4(self):
        self.assertEqual(self.receipt["freezeId"], "wp14r-resilient-development-v4")
        self.assertEqual(self.receipt["schemaVersion"], "4.0.0")
        self.assertEqual(
            self.receipt["protocol"]["protocolId"],
            "wp14r-supervised-scientific-job-v4",
        )
