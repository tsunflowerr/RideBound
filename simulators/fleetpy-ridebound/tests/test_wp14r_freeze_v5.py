...
_FREEZE_V5 = _ADAPTER / "wp14r_freeze_v5.py"
_RECEIPT = (
    _REPOSITORY
    / "benchmarks/scenarios/wp14r-development/freeze-v5-authorization.json"
)

_spec = importlib.util.spec_from_file_location("wp14r_freeze_v5_under_test", _FREEZE_V5)
...
class FreezeV5Tests(unittest.TestCase):
    def setUp(self):
        self.assertTrue(_RECEIPT.is_file(), "freeze v5 receipt must be built")
        self.receipt = json.loads(_RECEIPT.read_text(encoding="utf-8"))

    def test_receipt_is_superseded_and_no_longer_rebuildable(self):
        """Freeze v5 is a retained record: it ran 40 jobs, then the design moved.

        v5 closed the paired gate and ran 40 of 160 matrix jobs cleanly before
        halting on the first evening job. The halt was an adapter defect - an entered
        edge reported as the node behind it - not a
        protocol one, and fixing it changed a tree the scientific freeze v1
        sealed. The design was therefore re-frozen as freeze-receipt-v2.json and
        the protocol as v6, so v5 can no longer be rebuilt. The live regression
        lives in `test_wp14r_freeze_v6.py`.
        """
        completed = subprocess.run(
            [
                sys.executable, "-B", str(_FREEZE_V5),
        ...
        self.assertIn("WP14R_FREEZE_V5_ERROR", completed.stderr)

    def test_the_retained_receipt_bytes_are_untouched(self):
        raw = _RECEIPT.read_bytes()
        self.assertEqual(
            freeze.sha256_bytes(raw),
            "ba4a78358d1dcfbcdeba21d8ccad7bb2ff231d804fb05086e9737ca8980c1463",
        )
    ...
    def test_live_roots_are_v5_literal_paths(self):
    ...
    def test_identity_is_v5(self):
        self.assertEqual(self.receipt["freezeId"], "wp14r-resilient-development-v5")
        self.assertEqual(self.receipt["schemaVersion"], "5.0.0")
        self.assertEqual(
            self.receipt["protocol"]["protocolId"],
            "wp14r-supervised-scientific-job-v5",
        )
