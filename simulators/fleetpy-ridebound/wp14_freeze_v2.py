#!/usr/bin/env python3
"""Build and verify the WP14 development ablation freeze v2.

Identical scientific design to v1 - the same 16 cells, 10 arms, 160 jobs,
panel, denominators and seed. Only the sealed source differs.

Freeze v1 sealed an adapter that discarded a vehicle's commitment to the
edge it was driving on. The mapper floored edge progress to permille and
then, at zero, reported the vehicle as standing on the node behind it. On a
directed network that is not a conservative approximation but a different
place: the vehicle cannot un-enter an edge. Measured on the first evening
job of the v1 matrix, w14-d20181112-s10-r1-w17-b1-ref-s7: vehicle 4 was
0.126 permille along edge 86->8 and was reported as at node 86, so the core
costed 86->39 at 574.616 s while the true cost from its position was
792.244 s - reaching node 86 again costs 1352.586 s on its own. The
217.628 s gap turned 25.384 s of believed slack into a 192.244 s
pickup-window violation that FleetPy rejected outright.

The core was never wrong. RouteScheduleProjector already models an edge
position exactly: it adds the remaining edge time and continues from the
to-node. It simply never received one. The corrected mapper keeps the
vehicle on the edge and clamps to one permille, the smallest step the wire
contract admits and so the most conservative point it can express.

That source cannot be sealed by a receipt which recorded the defective tree,
so the design is re-frozen here. `wp14_freeze.py` and `freeze-receipt-v1.json`
are left exactly as they are: they remain the record of what the v1 matrix
actually ran under, including its halt.
"""
...
SCHEMA_ID = (
    "https://ridebound.local/schemas/wp14/v2/"
    "development-ablation-freeze.schema.json"
)
FREEZE_ID = "wp14-development-ablation-v2"
...
STATIC_REPOSITORY_FILES = {
    ...
    "simulators/fleetpy-ridebound/wp14_freeze_v2.py",
    ...
    "simulators/fleetpy-ridebound/tests/test_wp14_freeze_v2.py",
...
def main(argv=None):
    ...
    try:
        ...
        print(f"wp14_freeze_v2: {error}", file=sys.stderr)
        return 1
    return 0
