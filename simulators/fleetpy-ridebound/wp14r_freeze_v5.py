#!/usr/bin/env python3
"""Build and verify the exact pre-outcome WP14R protocol freeze v5.

Freeze v5 exists for the fourth defect of the same family as the first three: an
integration path nobody had walked until the paired gate actually passed.

Under freeze v4 both paired jobs succeeded and both verified independently — the
first time that gate was ever reached. Writing its receipt then failed, because
`paired-resource-gate-receipt.schema.json` pinned `freezeId` to the literal v2
identifier. The receipt could therefore never be written under any freeze after
v2. The protocol already binds `freezeId`, `freezeReceiptSha256` and
`protocolToolSha256` in code, so the schema constant added no guarantee at all;
it is now a version pattern.

v5 changes exactly two things: that schema field, and new `development-v5-*`
roots with the v4 pair joining the forbidden set. The scientific design is
untouched — the same byte-exact WP14-v1 base freeze, the same 160 jobs, the same
paired gate, the same host thresholds.

Measured under v4 before this freeze, and the reason it is worth continuing:

    B1  773.5 s   125,237,277 B
    C1  621.2 s   116,083,763 B     both independently valid
    projected 160-job matrix: 18,719,858,304 B = 87.17% of the 20 GiB cap,
    28.29 h sequential

C1 is faster and smaller than B1, which is what the constant-level skip
predicted: C1 had 97.40% redundant lexicographic passes against B1's 94.40%.
"""
...
SCHEMA_ID = (
    "https://ridebound.local/schemas/wp14r/v5/"
    "freeze-v5-authorization.schema.json"
)
SCHEMA_RELATIVE = pathlib.Path(
    "benchmarks/schemas/wp14r/v5/freeze-v5-authorization.schema.json"
)
...
FREEZE_ID = "wp14r-resilient-development-v5"
...
REPOSITORY_FILES = (
    ...
    "simulators/fleetpy-ridebound/wp14r_freeze_v5.py",
    ...
    "simulators/fleetpy-ridebound/tests/test_wp14r_freeze_v5.py",
...
class FreezeV5Error(RuntimeError):
...
def sha256_file(path):
    ...
    if not path.is_file():
        raise FreezeV5Error(f"required file not found: {path}")
...
def artifact(path, display_path=None):
    ...
    if not path.is_file() or path.is_symlink():
        raise FreezeV5Error(f"artifact is not a regular file: {path}")
    return {
...
def checked_artifact(path, expected_sha256, display_path=None):
    ...
    if value["sha256"] != expected_sha256:
        raise FreezeV5Error(f"artifact differs from the audited gate: {path}")
    return value
...
def load_module(name, path):
    ...
    if specification is None or specification.loader is None:
        raise FreezeV5Error(f"cannot load dependency: {path}")
...
def validate_schema(repository, receipt):
    ...
    try:
        ...
        raise FreezeV5Error(
            f"freeze-v5 schema is invalid: {error.message}"
        ) from error
    except jsonschema.ValidationError as error:
        raise FreezeV5Error(
            f"freeze-v5 receipt schema failed: {error.message}"
        ) from error
...
def verify_base_freeze(repository, base_path):
    ...
    if sha256_bytes(base_raw) != EXPECTED_BASE_RECEIPT_SHA256:
        raise FreezeV5Error("WP14-v1 freeze receipt hash changed")
    try:
        base = json.loads(base_raw)
    except json.JSONDecodeError as error:
        raise FreezeV5Error("WP14-v1 freeze receipt is invalid JSON") from error
    module = load_module(
        "wp14_freeze_for_wp14r_v5",
...
def protocol_for(base, host_fingerprint):
    ...
    if len(forbidden) != 11 or len(set(forbidden)) != 11:
        raise FreezeV5Error("isolation root set is not the exact eleven roots")
    if any(overlaps(LEDGER_ROOT, root) for root in forbidden):
        raise FreezeV5Error("ledger root overlaps retained or frozen evidence")
    if any(overlaps(CONTROL_ROOT, root) for root in forbidden):
        raise FreezeV5Error("control root overlaps retained or frozen evidence")
    if overlaps(LEDGER_ROOT, CONTROL_ROOT):
        raise FreezeV5Error("ledger and control roots overlap")
    job_ids = [job["jobId"] for job in base["design"]["jobs"]]
    if len(job_ids) != 160 or len(set(job_ids)) != 160:
        raise FreezeV5Error("base scientific job identity is not exact")
    if any(job_id not in job_ids for job_id in PAIR_JOB_IDS):
        raise FreezeV5Error("paired gate job is absent from the base freeze")
    return {
        "protocolId": "wp14r-supervised-scientific-job-v5",
...
def validate_mechanics_gate_provenance(repository, python_sha256):
    try:
        ...
        raise FreezeV5Error("mechanics gate artifact is invalid JSON") from error
    paths = {
    ...
    if not isinstance(expected, dict) or any(
        ...
        raise FreezeV5Error("current mechanics source differs from the 006 fixture")
    verifier = load_module(
        "wp14r_independent_verifier_for_freeze_v5_provenance",
    ...
    if (
        ...
        raise FreezeV5Error("current verifier/schema differs from the 006 gate")
    if fixture.get("pythonExecutableSha256") != python_sha256:
        raise FreezeV5Error("Python differs from the independently verified fixture")
...
def build(repository, authorized_at_utc, request_date="2026-08-28"):
    ...
    if not isinstance(authorized_at_utc, str) or not authorized_at_utc.endswith("Z"):
        raise FreezeV5Error("authorizedAtUtc must be an explicit UTC Z timestamp")
    try:
        datetime.date.fromisoformat(request_date)
    except (TypeError, ValueError) as error:
        raise FreezeV5Error("owner authorization date is invalid") from error
    base_path = repository / BASE_RECEIPT_RELATIVE
    base, base_raw = verify_base_freeze(repository, base_path)
    supervisor = load_module(
        "wp14r_supervisor_for_freeze_v5",
    ...
    if platform_name != "Windows":
        raise FreezeV5Error("freeze v5 is authorized only on the audited Windows host")
    host_module = load_module(
        "wp14r_host_preflight_for_freeze_v5",
    ...
    if host_module.read_active_power_scheme() != EXPECTED_POWER_SCHEME_GUID:
        raise FreezeV5Error("active power scheme differs from the v5 protocol")
    receipt = {
        "schemaVersion": "5.0.0",
        "schemaId": SCHEMA_ID,
        "recordType": "ridebound-wp14r-freeze-v5-authorization",
        "freezeId": FREEZE_ID,
    ...
    if receipt["baseScientificFreeze"]["artifact"]["sha256"] != (
        ...
        raise FreezeV5Error("base freeze bytes changed during v5 construction")
...
def read_canonical_receipt(path):
    ...
    try:
        ...
        raise FreezeV5Error("freeze-v5 receipt is not canonical JSON") from error
    if raw != canonical(document) + b"\n":
        raise FreezeV5Error("freeze-v5 receipt bytes are not canonical")
    return document, raw
...
def verify_receipt(path, repository=None):
    ...
    if actual != expected:
        raise FreezeV5Error("freeze-v5 receipt differs from its exact sources")
    return actual
    ...
    try:
        if arguments.write:
            if arguments.authorized_at_utc is None:
                raise FreezeV5Error("--write requires --authorized-at-utc")
            receipt = build(
        ...
        return 0
    except (OSError, FreezeV5Error) as error:
        print(f"WP14R_FREEZE_V5_ERROR: {error}", file=sys.stderr)
        return 2
