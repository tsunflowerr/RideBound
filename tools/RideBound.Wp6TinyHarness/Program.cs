...
static async Task<int> RunAsync(string[] arguments)
{
    try
    {
        ...
        var receipt = await TinyPairedHarness.RunAsync(
    ...
    catch (RideBound.Benchmarking.Bundles.BundleVerificationException exception)
    {
        Console.Error.WriteLine(
            $"wp6.tiny-harness-failed: stage={exception.Stage}; "
    ...
    {
        Console.Error.WriteLine(
            $"wp6.tiny-harness-failed: {exception.Message}");
        return 2;
    }
}
...
static IReadOnlyDictionary<string, string> Parse(string[] arguments)
{
    ...
    if (arguments.Length != required.Count * 2)
    {
        throw new ArgumentException(
            "usage: RideBound.Wp6TinyHarness --repository <root> --work-root <new-external-dir> --bundle <new-dir> --receipt <new-file> --configuration Debug|Release");
    }
    ...
    for (var index = 0; index < arguments.Length; index += 2)
    {
        ...
        {
            throw new ArgumentException("Tiny harness arguments are invalid or duplicated.");
        }
    }

    return values.Keys.ToHashSet(StringComparer.Ordinal).SetEquals(required)
        ? values
        : throw new ArgumentException("Tiny harness arguments are incomplete.");
}
